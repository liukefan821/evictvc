...
\begin{textabstract}
Verifiable LLM inference lets a client check that a model was executed faithfully on its input. Existing schemes commit to the entire retained activation state and cannot account for KV-cache eviction: the permanent deletion of token key/value states by streaming cache compressors such as StreamingLLM, H2O, and SnapKV. We introduce evictable vector commitments (EvictVC), a primitive whose committed positions can be permanently evicted under a public policy while a tamper-evident history of every deletion is kept. We formalize it with a syntax and three game-based security notions (eviction soundness, policy-binding, history tamper-evidence) and give a KZG-based construction in which an eviction is a single group operation independent of cache size, an audit proof for any set of positions is one group element, and all opening proofs are maintainable in quasi-linear time via the Feist-Khovratovich technique. We prove security under the n-SDH assumption and collision resistance. A BLS12-381 prototype confirms the asymptotics: eviction and updates cost about 135 microseconds irrespective of cache length up to 2^16, verification is two pairings (about 2.6 ms), and audit proofs are 48 bytes regardless of how many positions are audited.
\end{textabstract}
