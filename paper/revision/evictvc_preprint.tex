...
% Required for version=final; ignored in other modes.
\license{CC-by}
...
\title[running = {Evictable Vector Commitments}]{Evictable Vector Commitments: Verifiable LLM Inference under KV-Cache Eviction}

\addauthor[inst    = {1},
           orcid   = {0009-0004-9045-7879},
           email   = {KEFAN001@e.ntu.edu.sg},
           surname = {Liu}
          ]{Kefan Liu}
\addaffiliation[ror     = {02e7b5302},
                country = {Singapore}
...
\keywords{Vector commitments, KZG polynomial commitments, Verifiable computation, Machine-learning inference, KV-cache compression}
...
\begin{textabstract}
Verifiable LLM inference lets a client check that a model was executed faithfully on its input. Existing schemes commit to the entire retained activation state and therefore cannot account for KV-cache eviction: the permanent deletion of token key/value states performed by streaming cache compressors such as StreamingLLM, H2O, and SnapKV. We introduce evictable vector commitments (EvictVC), a commitment primitive whose committed positions can be permanently evicted under a public eviction policy while a tamper-evident history of every deletion is maintained. We formalize the primitive with a syntax and three game-based security notions (eviction soundness, policy-binding, and history tamper-evidence) and give a construction from KZG polynomial commitments in which an eviction is a single group operation independent of the cache size, an audit proof for an arbitrary set of positions is a single group element, and all opening proofs are maintainable in quasi-linear time via the Feist-Khovratovich technique. We prove security under the n-SDH assumption and the collision resistance of a hash function. A prototype over BLS12-381 confirms the asymptotics: eviction and commitment updates cost about 135 microseconds irrespective of cache length up to 2^16, verification is two pairings (about 2.6 ms), and audit proofs are 48 bytes regardless of how many positions are audited.
\end{textabstract}
...
\paragraph{Scope.}
EvictVC is a commitment primitive, not an end-to-end verifiable-inference
system. It takes the initial value and score commitments as given and certifies
the trajectory that follows: which positions were removed, under which policy
and threshold, carrying which values, and in what order. Whether $\mathbf{v}$
and $\mathbf{s}$ are in fact the key/value states and attention scores the model
computed lies outside the primitive; establishing that is the task of the
verifiable-inference layer surveyed in Section~\ref{sec:related}, with which
EvictVC composes by sharing the commitment. The distinction bears on what an
audit yields: an auditor who accepts every eviction in a trajectory learns that
the compression followed the stated policy, not that the state being compressed
was correct.
...
Fiore~\cite{PKC:CatFio13}; subsequent work added single-element subvector openings and
cross-commitment aggregation~\cite{SCN:TABDFK20} as well as efficient local updates.
We reuse single-element subvector openings to make batch audits constant-size.

Removing a position by updating it to zero is not new, and we do not claim it.
Catalano and Fiore already do exactly this: their universal dynamic accumulator
from vector commitments realises $\mathsf{Acc.Remove}$ as an update of position
$i$ from $1$ to $0$~\cite[\S6.1]{PKC:CatFio13}. What that construction does not
provide is what EvictVC adds. Its removal carries no precondition---the
accumulator's secret key is the empty string, so any holder of the state can
remove any element---it leaves no record of which removals occurred, and its
security notions are silent about them: position binding quantifies over two
openings at one position, and the accumulator's set binding treats the removals
requested through the oracle as defining the true set rather than asking whether
each was permitted. Table~\ref{tab:vccmp} states the comparison.
...
\begin{tabular}{@{}llll@{}}
\toprule
 & VC~\cite{PKC:CatFio13} & UDA~\cite[\S6.1]{PKC:CatFio13} & EvictVC \\
\midrule
Removing $i$        & ---              & $\mathsf{Update}(i,1\!\to\!0)$ & $\Evict(i,\tau)$ \\
Precondition        & none             & none              & $\Pol_\tau(s_i)=1$, public \\
State afterwards    & ---              & live, value $0$   & in $\Evicted$; $\Vfy$ rejects \\
Record kept         & none             & none              & chain $\eta$, epoch $t$ \\
Covered by security & position binding & set binding only  & Defs.~\ref{def:evsnd}--\ref{def:histte} \\
\bottomrule
\end{tabular}
\caption{EvictVC against the vector-commitment lineage it builds on. The update
mechanism is inherited; the precondition, the live/evicted distinction, and the
committed record of the operation sequence are what the security model adds.}
\label{tab:vccmp}
\end{table}
...
The Feist--Khovratovich technique~\cite{EPRINT:FeiKho23} computes all evaluation proofs of a
...
construction proves secure. The binding restriction is per-position rather than
threshold: the definition admits any efficiently computable antitone predicate,
and $\Pol_{<}$ is named only as the canonical instance.

Set-relative policies reduce to the per-position case once the threshold may
depend on the committed scores. Writing $s_{(1)}\le\dots\le s_{(m)}$ for the
scores of the live positions in sorted order, evicting the $k$ lowest is
$\Pol_{<}$ at $\tau=s_{(k+1)}$: the two agree exactly when the scores are
distinct, and at a boundary tie $s_{(k)}=s_{(k+1)}$ the threshold form removes
the positions strictly below, which is the part bottom-$k$ leaves unspecified.
What the reduction does not supply is a proof that the announced $\tau$ is that
order statistic; in the syntax below the threshold is supplied to eviction
verification rather than derived from the score commitment. Binding it would
require committing to a sorted permutation of the score vector and proving both
the permutation and the sortedness, or proving that exactly $k$ live scores lie
below $\tau$---comparison or range arguments that KZG openings do not
themselves provide. We treat set-relative policies as an extension
...
metadata. A verifier initialises $\Evicted\leftarrow\emptyset$ at $\Commit$ and
sets $\Evicted\leftarrow\Evicted\cup\{i\}$ on every eviction it has accepted, so
$\Evicted$ grows monotonically on the verifier's side and is never supplied by
the prover. Committer state $\aux$ holds the vectors, $\Live$, and $t$.
...
\item[$\Vfy(\pp,C,\Evicted,S,\{v_i\}_{i\in S},\pi)\to\bit$.] Outputs $1$ iff
      $S\cap\Evicted=\emptyset$ and the claimed values verify against $\Cval$.
...
\begin{remark}[Why $\Vfy$ consults $\Evicted$]\label{rem:live}
$\Evict$ leaves $\Cval$ committing to a vector whose $i$-th entry is $0$, so an
opening of an evicted position to $0$ satisfies $\VC.\Vfy$. Consulting $\Evicted$
separates ``the live value is $0$'' from ``the position is gone''. It also rules
out reinstatement: a prover may add $\delta L_i$ to $\Cval$ for $i\in\Evicted$,
and since $L_i$ vanishes at every other evaluation point this leaves openings at
live positions unchanged, so no check on $\Cval$ alone detects it. The check
$S\cap\Evicted=\emptyset$ rejects such positions, $\Evicted$ being maintained by
the verifier and monotone.
\end{remark}
...
$\Vfy(\pp,C,\Evicted,S,\{v_i\}_{i\in S},\Open(\pp,\aux,S))=1$, and $\Vfy$
outputs $0$ on every $S$ with $S\cap\Evicted\neq\emptyset$; and \emph{(ii)} every
...
Definition~\ref{def:vc} with the Lagrange-basis KZG commitment~\cite{AC:KatZavGol10} over a
...
Security rests on the $q$-Strong Diffie--Hellman assumption of Boneh and Boyen~\cite{EC:BonBoy04a}, stated in
...
\item[$\Open/\Vfy$.] $\VC.\Open$ on $\Cval$; $\Vfy$ rejects if
      $S\cap\Evicted\neq\emptyset$, otherwise runs $\VC.\Vfy_{\Val}$ on $\Cval$.
...
$S\subseteq\Live$ verifies by Lemma~\ref{lem:open}, and $\Vfy$ rejects any $S$
meeting $\Evicted$ before it consults $\Cval$. \emph{(ii)} If
...
Naively one opening costs $O(n\log n)$. By Feist--Khovratovich~\cite{EPRINT:FeiKho23}, all $n$
...
unique committed value at $i$ and $\Cval'=\Cval-\hat v L_i$, so position $i$
holds zero in $\Cval'$. The checks cover that step alone; a commitment published
later may differ from $\Cval'$ at $i$, and what carries irreversibility across
the trajectory is $\Evicted$ together with the liveness check in $\Vfy$
(Remark~\ref{rem:live}).
\end{remark}
